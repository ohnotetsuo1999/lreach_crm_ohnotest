...
export function ReminderEditor({
      ...
      >
          ...
          <div className="space-y-4">
            ...
            <TemplateConditionDetail
              ...
              tags={tags}
              statuses={statuses}
              onConditionUpdate={(condition) => updateTemplateTimingConfig(template.id, 'condition', condition)}
            />

            <FormActions>
              <Button variant="outline" onClick={() => setEditingTimingTemplateId(null)}>
                キャンセル
              </Button>
              <Button onClick={() => setEditingTimingTemplateId(null)}>
                保存
              </Button>
            </FormActions>
          </div>
  ...
  // Calculate if reminder is ready to save
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">
              {reminder ? 'リマインダーを編集' : '新規リマインダー作成'}
            </h1>
            <p className="text-sm text-gray-600 mt-1">
              予約や記念日に基づいて自動送信されるリマインダーを設定
            </p>
          </div>
        </div>
        <div className="flex items-center space-x-2">
